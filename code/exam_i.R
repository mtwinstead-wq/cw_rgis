...
mtcars_light_cy14<-filter(df_mtcars,wt<2.5&cyl==4)
...
mtcars_with_ptw<-filter(df_mtcars,cyl==6)
mtcars_with_ptw%>%arrange(desc(ptw))%>%filter(cyl==6)
...
df_mtcars%>%group_by(gear)%>%summarize(min_mpg=min(mpg),max_mpg=max(mpg))
...
g_scat<-ggplot(iris)+geom_point(mapping=aes(x=Sepal.Width,y=Sepal.Length))
...
g_bplot1<-PlantGrowth%>%ggplot(aes(x=group,y=weight,fill=group))+geom_boxplot()
